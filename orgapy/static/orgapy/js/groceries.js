function bindGroceries(groceriesContainer, saveButton, createListButton) {
    ...
    function fetchGroceriesData() {
        fetch(URL_API + "?action=list-groceries").then(res => res.json()).then(groceriesData => {
    ...
    function saveGroceriesData() {
        ...
        apiPost("save-groceries", {groceries: JSON.stringify(data)}, () => {
            toast("Saved", 600);
        });
    }
    ...
    function createList() {
        const data = readGroceriesData();
        apiPost("create-groceries-list", {groceries: JSON.stringify(data)}, (data) => {
